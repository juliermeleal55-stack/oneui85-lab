...

{
    ...
    # A commit timestamp makes generated configuration deterministic for a
    # given revision while SOURCE_DATE_EPOCH still permits reproducible CI.
    GET_BUILD_VAR "ROM_BUILD_TIMESTAMP" "${SOURCE_DATE_EPOCH:-$(git -C "$SRC_DIR" log -1 --format=%ct)}"
    ...
    GET_BUILD_VAR "SOURCE_DVFSAPP_CONFIG_SSRM_POLICY_FILENAME" "ssrm_default"
    GET_BUILD_VAR "TARGET_DVFSAPP_CONFIG_SSRM_POLICY_FILENAME" "ssrm_default"
